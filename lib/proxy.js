...
function KoaRouterProxy(context, options) {
    ...
    return async (ctx, next) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            logger_1.log(ctx.method, `${oldPath} -> ${targetUrl}`);
            /**
             * Constants
             */
            ...
            proxy.on('end', (req, res, proxyRes) => {
                logger_1.log(req.method, `${oldPath} -> ${targetUrl}`, res.statusCode, Date.now() - requestTime);
                // resolve promise
                resolve();
            });
            ...
            proxy.web(ctx.req, ctx.res, httpProxyOpts, (err, req, res, proxyUrl) => {
                logger_1.log(ctx.method, `${oldPath} -> ${targetUrl}`, 500, Date.now() - requestTime, err.message);
                reject(err);
            });
        });
    };
